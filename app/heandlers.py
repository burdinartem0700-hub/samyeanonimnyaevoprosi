...
from app.states import idea, Premium, Edits
        ...
        try:
            ...
            delte_key = InlineKeyboardMarkup(inline_keyboard=[
                ...
                [InlineKeyboardButton(text="Редактировать сообщение",
                                      callback_data=f"edit_{mesid.message_id}_{referrer_id}")]
            ])
            await message.reply("✅ Сообщение отправлено.", reply_markup=delte_key)
            await message.answer(
                f"💌Здесь вы можете получать анонимные сообщения!!\n\n"
                f"📎 Ваша  ссылка:\n{ref_link}\n\n"
                f"👥C помощью этой ссылки, люди смогут писать вам анонимные сообщения!!", reply_markup=send_url
            )
...
async def edits(callback: CallbackQuery, state: FSMContext):
    await state.set_state(Edits.eid)
    await state.update_data(eid = callback.data.split("_")[1], ed_user_id = callback.data.split("_")[2])
    await callback.message.answer("Напишите отредактированное сообщение: ", reply_markup=kb.back_send)
    await callback.message.edit_reply_markup(reply_markup=None)

@router.callback_query(F.data == "back_sen")
async def send_back(callback: CallbackQuery, state: FSMContext):
